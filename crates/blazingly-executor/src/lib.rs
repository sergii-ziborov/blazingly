...
use blazingly_core::{
    ...
    BodyStreamError, Cookie, Created, File, Form, Header, HttpMethod, HttpUpgrade, InputDescriptor,
    InputSource, Json, MAX_MULTIPART_HEADER_BYTES, MAX_MULTIPART_PARTS, Multipart, MultipartError,
...
/// request line â€” an MCP tool call â€” rejects deterministically with
...
///
/// Generated by `#[provider]`, which is the only place the input's concrete
/// type is known; the executor stores and calls these without ever naming an
/// extractor.
#[doc(hidden)]
pub type ProviderInputDecoder =
    Rc<dyn Fn(&InvocationInput<'_>) -> Result<DependencyValue, InputRejection>>;

/// One request input a provider declared alongside its dependencies.
#[doc(hidden)]
pub struct ProviderInput {
    descriptor: InputDescriptor,
    value_type: core::any::TypeId,
    decode: ProviderInputDecoder,
}

impl ProviderInput {
    /// Records one declared input and its generated decoder.
    #[doc(hidden)]
    #[must_use]
    pub fn new<T: 'static>(descriptor: InputDescriptor, decode: ProviderInputDecoder) -> Self {
        Self {
            descriptor,
            value_type: core::any::TypeId::of::<T>(),
            decode,
        }
    }
}
...
impl RequestProvider {
    /// Pairs a compiled provider with its declared request inputs.
    #[doc(hidden)]
    #[must_use]
    pub fn new(provider: Provider, inputs: Vec<ProviderInput>) -> Self {
        Self { provider, inputs }
    }

    fn key(&self) -> blazingly_di::DependencyKey {
        self.provider.key()
    }

    fn lifetime(&self) -> DependencyLifetime {
        self.provider.lifetime()
    }

    fn dependencies(&self) -> &[blazingly_di::DependencyKey] {
    ...
    ) -> Result<CompiledProvider, blazingly_di::ProviderCompileError> {
        self.provider.compile(slots)
    }
}

impl From<Provider> for RequestProvider {
...
impl ExecutableOperation {
    ...
    async fn invoke_pipeline(&self, input: InvocationInput<'_>) -> ExecutionOutcome {
        ...
        let seeded = match plan.decode_inputs(input) {
            Ok(seeded) => seeded,
            Err(rejection) => return rejection.into_execution_outcome(),
        };
        let requests = match plan.resolve(seeded).await {
    ...
    async fn invoke_pipeline_controlled(
        ...
        let requests = match plan.resolve_controlled(seeded, control).await {
            Ok(requests) => requests,
...
struct CompiledOperationDependencies {
    ...
    /// Decoders for provider-declared request inputs, one per compiled input
    /// slot. Inputs occupy the first `input_decoders.len()` request slots, so
    /// provider `i`'s value lives at `input_decoders.len() + i`.
    input_decoders: Vec<ProviderInputDecoder>,
    sync_chain: Cell<u8>,
}
...
impl CompiledOperationDependencies {
    ...
    const fn slot_base(&self) -> usize {
        self.input_decoders.len()
    }

    /// Decodes every provider-declared request input into its slot.
    ///
    /// Runs before the provider chain, so a provider never observes a missing
    /// input; a decode failure is the same typed rejection the equivalent
    /// handler argument would have produced, not an internal error.
    fn decode_inputs(
        &self,
        input: InvocationInput<'_>,
    ) -> Result<RequestDependencyValues, InputRejection> {
        let mut requests =
            RequestDependencyValues::new(self.slot_base() + self.request_providers.len());
        for (index, decode) in self.input_decoders.iter().enumerate() {
            let value = decode(&input)?;
            requests.set(index, value).map_err(|error| {
                InputRejection::new(500, "invalid_input_slot", error.to_string())
            })?;
        }
        Ok(requests)
    }
    ...
    async fn resolve(
        ...
        for (index, provider) in self.request_providers.iter().enumerate() {
            match provider.run_sync(&self.singletons, requests.as_slice()) {
                Ok(value) => requests.set(base + index, value)?,
...
impl Plugin {
    ...
    /// application is compiled, so one module â€” a function returning a
    /// `Plugin` â€” can be mounted at two prefixes by calling it twice, without
    ...
    #[must_use]
    pub fn provide(mut self, provider: impl Into<RequestProvider>) -> Self {
...
impl TestOverrides {
    ...
    fn apply(&mut self, plugin: &str, providers: Vec<RequestProvider>) -> Vec<RequestProvider> {
        let mut providers = providers
            ...
            .map(|provider| {
                ...
                // A replacement is a plain provider: the mock supplies the
                // value itself, so the replaced provider's request inputs are
                // neither decoded nor required.
                replacement.map_or(provider, |index| {
                    let entry = &mut self.entries[index];
                    entry.applied = true;
                    RequestProvider::from(entry.provider.clone())
                })
            })
            .collect::<Vec<_>>();
        ...
        for index in 0..self.entries.len() {
            ...
            let entry = &mut self.entries[index];
            entry.applied = true;
            providers.push(RequestProvider::from(entry.provider.clone()));
        }
        providers
    }
...
impl fmt::Display for ExecutableBuildError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SingletonRequestInputs { plugin, dependency } => write!(
                formatter,
                "singleton provider `{dependency}` in plugin `{plugin}` cannot declare request \
                 inputs; a singleton is built once at compile time, before any request exists"
            ),
            Self::ConflictingProviderInput {
                operation,
                name,
                source,
            } => write!(
                formatter,
                "operation `{operation}` receives {source} input `{name}` at two different \
                 types; one wire input cannot decode as both"
            ),
        }
    }
}
...
impl ExecutableApp {
    ...
    pub fn from_plugin_with_overrides(
        ...
        for mut scoped in scoped_operations {
            // Provider-declared request inputs fold into the operation
            // contract before the definition is built, so path placeholders,
            // duplicate names, projections, and fingerprints all see them as
            // if the handler had declared them itself.
            let input_plan = plan_operation_inputs(&scoped, &registrations)?;
            scoped
                .operation
                .descriptor
                .contract
                .inputs
                .extend(input_plan.folded.iter().cloned());
...
fn collect_plugin(
    ...
    for provider in providers {
        ...
        if provider.lifetime() == DependencyLifetime::Singleton && !provider.inputs.is_empty() {
            return Err(ExecutableBuildError::SingletonRequestInputs {
                plugin: path,
                dependency: key.type_name(),
            });
        }
...
///
/// Both nest â€” a mounted plugin inside a mounted plugin serves under the
/// joined prefix and the dotted namespace â€” and both are applied to each
...
/// The request inputs one operation's provider chain declared, compiled to
/// slots.
///
/// Inputs occupy the first `decoders.len()` request slots, deduplicated by
/// wire identity and decoded type, so the same header consumed by two
/// providers is decoded once. `folded` is the subset of descriptors the
/// handler did not already declare, appended to the operation contract so
/// every projection â€” `OpenAPI` parameters, MCP tool schemas, generated
/// documentation, fingerprints â€” sees a dependency-origin input exactly like
/// a handler-declared one.
struct OperationInputPlan {
    decoders: Vec<ProviderInputDecoder>,
    folded: Vec<InputDescriptor>,
    provider_inputs: HashMap<usize, Vec<usize>>,
}

fn plan_operation_inputs(
    scoped: &ScopedOperation,
    registrations: &[ProviderRegistration],
) -> Result<OperationInputPlan, ExecutableBuildError> {
    let mut plan = OperationInputPlan {
        decoders: Vec::new(),
        folded: Vec::new(),
        provider_inputs: HashMap::new(),
    };
    let mut slots: HashMap<(&'static str, String), (core::any::TypeId, usize)> = HashMap::new();
    let mut ordered: Vec<InputDescriptor> = Vec::new();
    let mut visited = vec![false; registrations.len()];
    let mut pending = Vec::new();
    for request in &scoped.operation.dependency_requests {
        if let Some(id) = scoped.visible.get(&request.key().type_id()).copied() {
            pending.push(id);
        }
    }
    while let Some(id) = pending.pop() {
        ...
        visited[id] = true;
        let registration = &registrations[id];
        // A singleton is built before any request exists; its subtree was
        // already rejected if anything in it declared an input.
        if registration.provider.lifetime() == DependencyLifetime::Singleton {
            continue;
        }
        let mut positions = Vec::with_capacity(registration.provider.inputs.len());
        for input in &registration.provider.inputs {
            let key = (
                source_name(input.descriptor.source),
                input.descriptor.name.clone(),
            );
            let slot = if let Some((existing_type, slot)) = slots.get(&key) {
                if *existing_type != input.value_type {
                    return Err(ExecutableBuildError::ConflictingProviderInput {
                        operation: scoped.operation.descriptor.contract.id.as_str().to_owned(),
                        name: input.descriptor.name.clone(),
                        source: source_name(input.descriptor.source),
                    });
                }
                *slot
            } else {
                let slot = plan.decoders.len();
                slots.insert(key, (input.value_type, slot));
                plan.decoders.push(Rc::clone(&input.decode));
                ordered.push(input.descriptor.clone());
                slot
            };
            positions.push(slot);
        }
        if !positions.is_empty() {
            plan.provider_inputs.insert(id, positions);
        }
        for dependency in registration.provider.dependencies() {
            if let Some(dependency_id) = registration.visible.get(&dependency.type_id()).copied() {
    ...
    plan.folded = ordered
        ...
        .filter(|descriptor| {
            !scoped
                ...
                .iter()
                .any(|declared| {
                    declared.source == descriptor.source && declared.name == descriptor.name
                })
        })
        .collect();
    Ok(plan)
...
fn compile_operation_dependencies(
    ...
    Ok(CompiledOperationDependencies {
        ...
        request_providers,
        handler_slots,
        input_decoders: plan.decoders.clone(),
        sync_chain: Cell::new(CHAIN_UNKNOWN),
    })
}
...
fn compile_request_provider(
    ...
    // Declared request inputs follow the dependencies, in declaration order,
    // each pointing at its pre-decoded slot at the front of the request slice.
    if let Some(positions) = plan.provider_inputs.get(&id) {
        for position in positions {
            dependency_slots.push(DependencySlot::Request(*position));
        }
    }
    ...
    let slot = plan.decoders.len() + request_providers.len();
...
thread_local! {
    ...
    /// and a `Vec` per rule list â€” around thirty allocations for a
...
mod blocking_pool_tests {
    ...
    fn resolve_plan(
        plan: &CompiledOperationDependencies,
    ) -> Result<crate::RequestDependencyValues, DependencyError> {
        let seeded = crate::RequestDependencyValues::new(plan.request_providers.len());
        futures_lite::future::block_on(plan.resolve(seeded))
    }
    ...
    fn an_all_sync_chain_resolves_without_awaiting_and_stays_classified() {
        ...
        for _ in 0..2 {
            let values = resolve_plan(&plan).expect("chain resolves");
    ...
    fn a_synchronous_provider_failure_is_reported_rather_than_retried() {
        ...
        let Err(error) = resolve_plan(&plan) else {
