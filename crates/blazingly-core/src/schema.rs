...
//! the other. The few decisions the formats genuinely differ on are supplied
//! by the caller through [`SchemaDialect`].
//!
...
pub trait SchemaDialect {
    ...
    ///
    /// [`apply_validation`] recovers declarative `keyword=value` constraints
    /// itself; every other validator is offered to the dialect and recorded
    /// under `x-blazingly-validators` only when this hook declines it.
    fn project_custom_validator(&self, _schema: &mut Value, _validator: &str) -> bool {
        false
    }
}

/// Projects one value type into its schema node.
#[must_use]
pub fn schema_value(dialect: &impl SchemaDialect, descriptor: &TypeDescriptor) -> Value {
    if let Some(model) = &descriptor.model {
        return dialect.model_node(descriptor, model);
    }
...
///
/// Declarative `keyword=value` constraints are projected as real JSON Schema
