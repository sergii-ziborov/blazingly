...
| --- | --- | --- | --- | --- |
| P0 | Request-aware dependency injection | FastAPI dependencies can declare request parameters and subdependencies | **Shipped 2026-08-04**: `#[provider]` accepts `Path`/`Query`/`Header`/`Cookie` beside `Depends`, sync/async/transient; inputs fold into the operation contract exactly once (deduplicated across providers), fail validation like handler inputs before any provider runs, and overrides bypass the replaced provider's inputs. Remaining: `Extension<T>` provider inputs, and the 1/10-dependency benchmark rows | Nested providers consume typed request inputs; those inputs appear once in the operation contract, OpenAPI, MCP metadata, and generated docs; overrides bypass the replaced provider and its inputs |
| P0 | Custom extraction | Axum exposes `FromRequest`; FastAPI permits direct request access | **Shipped 2026-08-03**: `Extract<T>` names a downstream `FromInvocation` impl explicitly, `HttpRequestParts` exposes borrowed request-line/connection accessors, and `Extract<RequestParts>` snapshots them; a transport without a request line rejects `400 transport_mismatch`. Remaining: a borrowed (non-snapshot) parts value for synchronous handlers | A downstream crate implements `FromInvocation` and uses `Extract<T>` in an HTTP handler; a borrowed raw request-parts extractor is available; unsupported transports reject deterministically |
| P0 | Reusable API composition | FastAPI `APIRouter` and Axum `Router` support prefixes, nesting, defaults, and reuse | **Shipped 2026-08-03**: `Plugin::mount` joins nested path prefixes at compile time and `with_id_namespace` keeps identities, sections, and MCP tool names distinct per mount, with build-time rejection of malformed or colliding mounts. Remaining: default policies and per-mount tags beyond the id namespace | The same module mounts under two prefixes without cloning handlers; duplicate paths and operation ids fail at build time; OpenAPI/MCP paths, tags, dependencies, and policies match the mounted graph |
